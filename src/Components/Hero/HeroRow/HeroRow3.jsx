...
const HeroRow3 = () => {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2  lg:grid-cols-5 gap-6 p-6 bg-custom-white">
      {products.map((product, index) => (
        ...
        >
          {product.tag && (
            <span className="absolute top-2 left-2 -ml-7 px-2 py-1 text-xs font-semibold rounded-md text-white bg-custom-tag">
              {product.tag}
            </span>
          )}
          <img
            src={product.image}
            alt={product.title}
            className="w-[200px] font-bold font-quicksand h-[180px] object-contain mx-auto mb-4"
          />
          <p className="text-xs opacity-100 text-custom-text-lightgray font-lato  mb-1">{product.category}</p>
          <h3 className="font-bold font-quicksand text-md mb-1">{product.title}</h3>
          <p className="  text-custom-text-lightgray mb-1">
  <span className="mr-20 font-lato">⭐️</span>{product.rating}
</p>


          <p className="text-xs text-gray-400  mb-1">By <span className="text-green-600">{product.brand}</span></p>
          <div className="flex items-center justify-between mt-2">
            <p className="text-lg font-bold px-2 text-custom-green">${product.price.toFixed(2)}</p>
            <p className="text-sm line-through text-gray-400">${product.oldPrice.toFixed(2)}</p>
            <button className="ml-auto bg-custom-secondary text-white px-3 py-1 rounded-lg text-sm hover:bg-opacity-80">Add</button>
          </div>
        </div>
      ))}
    </div>
